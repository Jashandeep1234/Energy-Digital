...
import { LayoutDashboard, BarChart3, Building2, BrainCircuit, Settings, LogOut, Activity, Menu, X } from "lucide-react";
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 lg:hidden z-50 w-12 h-12 flex items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 to-blue-600 shadow-[0_0_20px_rgba(6,182,212,0.4)] text-white hover:shadow-[0_0_30px_rgba(6,182,212,0.6)] transition-all"
      >
        {isOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
      </button>

      {/* Desktop Sidebar */}
      <div className="hidden lg:flex fixed left-0 top-0 h-screen w-20 flex flex-col items-center py-10 bg-black/40 border-r border-white/5 backdrop-blur-3xl z-[100] group transition-all duration-700 ease-[0.16, 1, 0.3, 1] hover:w-72 overflow-hidden shadow-[30px_0_60px_rgba(0,0,0,0.5)]">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-20 right-0 lg:hidden w-80 max-w-[calc(100vw-2rem)] bg-black/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6 z-40"
          >
            <h3 className="text-xs font-black uppercase tracking-widest text-white/40 mb-4">Navigation</h3>
            <div className="space-y-3">
              {navItems.map((item) => (
                <button
                  ...
                  className={cn(
                    "flex items-center w-full px-4 py-3 rounded-xl transition-all duration-300",
                    activeTab === item.id 
              ...
              <button
                ...
                className="flex items-center w-full px-4 py-3 rounded-xl text-white/60 hover:text-white hover:bg-white/5 transition-all"
              >
                <Settings className="h-5 w-5 mr-3" />
                <span className="text-sm font-black uppercase tracking-wide">Settings</span>
              </button>
              <button
                onClick={() => {
                  onLogout();
                ...
                className="flex items-center w-full px-4 py-3 rounded-xl text-red-400/60 hover:text-red-400 hover:bg-red-500/10 transition-all"
              >
